...
const App: React.FC = () => {
  ...
  // --- Client Identification State (Positional) ---
  const [clientPosition, setClientPosition] = useState<ClientPosition | null>(null);
  ...
  const hasImageFiles = files.some(f => f.file.type.startsWith('image/'));
  ...
  useEffect(() => {
      // Reset client position if image files are removed
  ...
  useEffect(() => {
    ...
    const clientSelectionPending = hasImageFiles && !clientPosition;
  ...
  }, [report, currentLitigationStage, currentLitigationType, files, query, isLoading, hasImageFiles, clientPosition]);
  ...
  useEffect(() => {
    if (isLoading) {
        ...
        progressIntervalRef.current = window.setInterval(() => {
            let increment = 1;
            if (currentProgress < 20) increment = 1;
            else if (currentProgress < 80) increment = 2;
            else increment = 1;
            ...
            if (isReanalyzing) {
                ...
                setProgressMessage('AI đang phân tích các vấn đề pháp lý cốt lõi...');
            } else if (currentProgress < 95) {
                setProgressMessage('AI đang xây dựng chiến lược và đề xuất...');
            } else {
                setProgressMessage('Đang tổng hợp báo cáo cuối cùng...');
            }
        }, 400);
  ...
  const performMainAnalysis = useCallback(async (filesToAnalyze: UploadedFile[], clientSide: ClientPosition | null) => {
    setIsLoading(true);
    setReport(null);
    setOriginalReport(null);
    setError(null);
  ...
  const handleStartFileProcessing = useCallback(async () => {
    ...
    if (files.length > 0) {
        ...
        await performMainAnalysis([], clientPosition);
    }
  }, [files, performMainAnalysis, clientPosition]);
  
  const handleContinueAnalysis = useCallback(async () => {
    if (hasImageFiles && !clientPosition) {
        setError("Vui lòng chọn vị trí của thân chủ trong tin nhắn (Trái/Phải) trước khi phân tích.");
        setIsProcessing(false); // Stop the processing flow
        return;
    }
    const successfulFiles = files.filter(f => f.status === 'completed');
    setIsProcessing(false);
    await performMainAnalysis(successfulFiles, clientPosition);
  }, [files, performMainAnalysis, clientPosition, hasImageFiles]);
  ...
  const handleExtractAndDisplaySummaries = useCallback(async () => {
    if (files.length === 0) {
     ...
     if (hasImageFiles && !clientPosition) {
        setSummaryError("Vui lòng chọn vị trí của thân chủ trong tin nhắn (Trái/Phải) trước khi tóm tắt.");
        return;
    }
  ...
  }, [files, clientPosition, hasImageFiles]);
  ...
  }, [originalReport, currentLitigationType, currentLitigationStage, files, query, clientPosition]);

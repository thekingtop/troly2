...
export default function App() {
    ...
    const handleNewCase = (workflowType: 'consulting' | 'litigation') => {
      setActiveCase({
        id: `new_${Date.now()}`,
        name: '',
        workflowType,
    ...
    const CaseSelectionScreen: React.FC = () => (
      <div className="min-h-screen bg-slate-100 flex flex-col items-center p-4 sm:p-6 md:p-8">
          ...
          <div className="w-full max-w-4xl bg-white p-6 rounded-xl shadow-lg border border-slate-200/80">
                          ...
                          savedCases.map(c => (
                              <div key={c.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg hover:bg-slate-100/50 hover:shadow-sm transition-all">
                                  <div onClick={() => handleSelectCase(c)} className="cursor-pointer flex-grow min-w-0">
                                      ...
                                      <p className="text-xs text-slate-500">{c.workflowType === 'litigation' ? 'Vụ việc Tranh tụng' : 'Nhiệm vụ Tư vấn'} - Cập nhật: {new Date(c.updatedAt).toLocaleString('vi-VN')}</p>
                                  </div>
